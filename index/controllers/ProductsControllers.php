...
class ProductsControllers extends AuthControllers
{
	...
	public function index()
	{
		...
		if ($_POST) {
		    ...
		    $this->smarty->assign('project_classification',$project_classification);
	      	...
	      	$this->smarty->assign('products_download_by_project_index',"/".root_filename.".php?".AuthCode("m=Products&a=download_by_project_index","ENCODE",$_SESSION['domain_key']));
	...
	public function add_index()
	{
		...
		if($_POST){
	        ...
	        foreach ($data as $k => $v) {
		        ...
		        $db->bind("repair_content", "");
		        $info = $db->query("INSERT INTO domain_post(title,session,content,bugDetail,creation_time,update_time,cate_id,user_id,bugLevel,company,description,suggestions,repair_content) VALUES (:title,:session,:content,:bugDetail,:creation_time,:update_time,:cate_id,:user_id,:bugLevel,:company,:description,:suggestions,:repair_content)");
		        if($info){
				...
				if(file_exists(ROOT_PATH."/public/docx/".$_SESSION['user_info']['uuid']."/".$file_path_name)){
					...
					if($post){
						...
						foreach ($company as $k => $v) {
						    ...
						    try {
								$sw = new stockConnector(isset($_SESSION['system_config']['socket_ip'])?$_SESSION['system_config']['socket_ip']:'127.0.0.1',"5671");
	 ...
	 * @return html
	 */
	public function download_by_project_index()
	{
		$this->jurisdiction("非法访问按项目导出漏洞报告");
		$this->log_db("用户访问按项目导出漏洞报告","7");
		$db = $this->Db();
		if($_POST){
			$company_id = isset($_POST['company_id']) ? intval($_POST['company_id']) : '';
			$path = isset($_POST['path']) ? $_POST['path'] : '';
			$token = isset($_POST['token']) ? $_POST['token'] : '';
			$session_token = isset($_SESSION['token']) ? $_SESSION['token'] : '';
			if(empty($company_id)) $this->json(['status'=>0,'msg'=>'请选择项目！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			if(empty($path)) $this->json(['status'=>0,'msg'=>'输入模板ID！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			if(empty($token)) $this->json(['status'=>0,'msg'=>'输入token！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			if(empty($session_token)) $this->json(['status'=>0,'msg'=>'token异常！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			if($token != $session_token) $this->json(['status'=>0,'msg'=>'token验证失败！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			...
			$token = md5(code().time().code());
			$_SESSION['token'] = $token;
			$this->json(['status'=>1,'msg'=>'正在导出！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=download_by_project_index&company_id=".$company_id."&token=".$token."&path=".$path,"ENCODE",$_SESSION['domain_key'])]]);
		}
		if($_GET){
			$path = isset($_GET['path']) ? $_GET['path'] : '';
			$company_id = isset($_GET['company_id']) ? intval($_GET['company_id']) : '';
			$token = isset($_GET['token']) ? $_GET['token'] : '';
			$session_token = isset($_SESSION['token']) ? $_SESSION['token'] : '';
			if(empty($company_id)) $this->json(['status'=>0,'msg'=>'请选择项目！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			if(empty($path)) $this->json(['status'=>0,'msg'=>'输入模板ID！',"data"=>["url"=>"/".root_filename.".php?".AuthCode("m=Products&a=index","ENCODE",$_SESSION['domain_key'])]]);
			...
			if($list){
				...
				if(file_exists(ROOT_PATH."/public/docx/".$_SESSION['user_info']['uuid']."/".$file_path_name)){
					...
					$sql = "select a.title,a.content,a.bugLevel,a.bugDetail,a.description,a.suggestions,b.title as realname,b.id,c.title as cate_id from domain_post as a left join domain_project_classification as b on a.company = b.id left join domain_classification as c on a.cate_id = c.id where a.company = :company";
					if($_SESSION['userid'] != '1'){
						$db->bind("user_id", $_SESSION['userid']);
						$sql .= " AND a.user_id = :user_id";
					}
					...
					if($post){
						...
						$zong_shu = [];
						$risk_level = '';
						$high = 0; $medium = 0; $low = 0; $serious = 0;
						$tmp_img = [];
						...
						foreach ($post as $k => $v) {
							...
							if($v['bugLevel'] == 2){ $zong_shu[$v['realname']]['low'] += 1; $low += 1;
							} elseif($v['bugLevel'] == 3){ $zong_shu[$v['realname']]['medium'] += 1; $medium += 1;
							} elseif($v['bugLevel'] == 4){ $zong_shu[$v['realname']]['high'] += 1; $high += 1;
							} elseif($v['bugLevel'] == 5){ $zong_shu[$v['realname']]['serious'] += 1; $serious += 1; }
							$post[$k]['content'] = $array;
							@$company[$v['realname']][] = [
								'id'=>count($company[$v['realname']])+1,
								'pathname'=>$v['title'],'name'=>$v['title'],'repair_time'=>3,
								'level'=>intval($v['bugLevel']),'url'=>$v['bugDetail'],
								'analysis'=>$v['description'],'verification'=>$array,'suggestions'=>$v['suggestions'],
							];
							@$hostlist[$v['realname']][] = ['id'=>count($hostlist[$v['realname']])+1,'url'=>$v['bugDetail'],'name'=>$v['title'],'type'=>$v['cate_id'],'bugLevel'=>$v['bugLevel']];
							@$vulnerability_types[$v['realname']][] = $v['cate_id'];
							$url[] = @getTopHost($v['bugDetail']);
						}
						if($serious>=1 || $high >= 3){ $risk_level = "极度风险";
						} elseif ($high > 0 || $medium >= 5){ $risk_level = "严重风险";
						} elseif ($medium >= 2){ $risk_level = "严重隐患";
						} else { $risk_level = "一般隐患"; }

						$alerts = [];
						foreach ($company as $k => $v) { $alerts[$k] = ['name'=>$k,'path'=>$v]; }
						foreach ($url as $k => $v) { $url[$k] = "*.".$v; }
						$url = array_unique($url);
						reset($company);
						include_once ROOT_PATH."/lib/socket/stockConnector.php";
						$file_path = [];
						foreach ($company as $k => $v) {
							$zong_data = [
								'name'=>$k,'doctype'=>1,'time'=>date("Y年m月d日"),
								'producer'=>isset($_SESSION['name']) ? $_SESSION['name'] : 'admin',
								'producer_time'=>date("Y.m.d"),'reviewer'=>"","reviewer_time"=>"",
								'url'=>implode(",", $url),'hostlist'=>$hostlist[$k],
								'alerts'=>[$alerts[$k]],
								'low'=>$zong_shu[$k]['low'],'medium'=>$zong_shu[$k]['medium'],
								'high'=>$zong_shu[$k]['high'],'serious'=>$zong_shu[$k]['serious'],
								'risk_level'=>$risk_level,
								'common'=>$zong_shu[$k]['low']+$zong_shu[$k]['medium']+$zong_shu[$k]['high']+$zong_shu[$k]['serious'],
								'vulnerability_types'=>implode(",", array_unique($vulnerability_types[$k])),
							];
